...
function abc_booking_editCalendar() {
	...
	if ( isset($_POST["id"]) && isset($_POST["name"]) && isset($_POST["maxAvailabilities"]) 
		...
		$er = $wpdb->get_row('SELECT SUM(maxAvailabilities) as rooms FROM '.$wpdb->prefix.'abc_calendars', ARRAY_A);
		$rooms = $er["rooms"];
		$er = $wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'abc_calendars WHERE id = '.intval($_POST["id"]), ARRAY_A);
...
function abc_booking_delCalendar() {
	...
	if ( isset($_POST["id"]) ) {
		...
		if($er["bookings"] > 0){ // There are still bookings for this calendar. Calendar can not be deleted.
			wp_redirect(  admin_url( "admin.php?page=advanced-booking-calendar-show-seasons-calendars&setting=calNotDeleted&calId=".intval($_POST["id"]) ) );			
		}else{
...
function advanced_booking_calendar_show_seasons_calendars() {
	...
	if(isset($_GET["setting"]) ){
		switch($_GET["setting"]){
			...
			case 'changeSaved':
				...
				if(isset($_GET["room"])){
					if($_GET["room"] == 0){
						...
						$notices .= sprintf( __('Rooms reduced to %s rooms', 'abc-bookings'), esc_html($_GET["room"]));
					}
				}		
			...
			case 'calDeleted':
				...
				break;
			case 'calNotDeleted':
				$deleteLinkStart = '<form method="post" class="uk-form uk-form-horizontal" action="admin-post.php">
				<input type="hidden" name="action" value="abc_booking_delCalendarForced" />
				<input type="hidden" name="calId" value="'.intval($_GET['calId']).'" />';
				$deleteLinkStart = '<a href="'.admin_url( "admin.php?page=advanced-booking-calendar-show-seasons-calendars&action=abc_booking_delCalendarForced&calId=".intval($_GET['calId'])).'">';
				$deleteLinkEnd = '</a>';

				$notices .= '<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible"> 
							<p><strong>'.__('Calendar was not deleted. There still exist bookings in this calendar.', 'advanced-booking-calendar' ).'</strong></p>
							<p><strong>'.sprintf( __('If you want to delete the calendar and all its bookings, please click %shere%s.', 'advanced-booking-calendar' ),
										$deleteLinkStart,
										$deleteLinkEnd)
										.'</strong></p>
							<button type="button" class="notice-dismiss">
								<span class="screen-reader-text">'.__('Dismiss this notice.', 'advanced-booking-calendar').'</span>
							</button></div>';
				break;
			case 'calAdded':
				$notices .= '<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible"> 
	...
	switch($getAction){
		case "abc_booking_delCalendarForced":
			if ( isset($_GET["calId"]) ) {
				$calId = intval($_GET["calId"]);
				$wpdb->delete($wpdb->prefix.'abc_calendars', array('id' => $calId));
				$wpdb->delete($wpdb->prefix.'abc_rooms', array('calendar_id' => $calId));
				$wpdb->delete($wpdb->prefix.'abc_seasons_assignment', array('calendar_id' => $calId));
				$wpdb->delete($wpdb->prefix.'abc_bookings', array('calendar_id' => $calId));
				abc_booking_setPersonCount();
				echo "<p>".__('Calendar and all its bookings have been deleted.', 'advanced-booking-calendar')."</p>";
			}else{
				echo "Calendar ID missing.";
			}
			break;
